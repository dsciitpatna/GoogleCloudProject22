...
class EventCreation(APIView):
    ...
    def post(self, request):
        ...
        if user_id is '' or User.objects.get(organisation = org_id, id = user_id):
            return Response(
                {"status":"error",
                    "Message": "User doesn't exist within organisation"}, 
                status=status.HTTP_401_UNAUTHORIZED
            )
        serializer.data['organisation'] = org_id
        serializer.data['created_by'] = user_id
        if serializer.is_valid():
            if request.data['_type'] == 1:
                serializer.save()
...
class TagView(APIView):
    def get(self, request):
        list = []
        for i in Tag.objects.values_list('tag'):
            i = i[0]
            list.append({i:Tag.objects.get(tag=i)})
