...
from .utility import Autherize, EmailSending, isauthenticated
...
class UserGetView(APIView):
    def get(self, request, **kwargs):
        ...
        if request.COOKIES.get('jwt') is None:
            ...
            default_user = isauthenticated(request)
            ...
            token = jwt.encode(
                payload, COOKIE_ENCRYPTION_SECRET, algorithm='HS256')
            response = Response(
                {"role": user.role, "user_id": uid}, status=status.HTTP_200_OK)
            ...
            return Response({"role": user.role, "user_id": user.userid}, status=status.HTTP_200_OK)
...
class ClubAdminView(APIView):
    ...
    def get(self, _, **kwargs):
        ...
        club_admins = User.objects.filter(
            role="1", organization=user.organization)
        data = []
    ...
    def put(self, request, **kwargs):
        ...
        try:
            club_admin = User.objects.get(
                userid=_userid, organization=user.organization, role="1")
...
class EmailVarification(APIView):
    def get(self, request, token):
        ...
        try:
            payload = jwt.decode(
                token, COOKIE_ENCRYPTION_SECRET, algorithms=['HS256'])
...
class OauthHelper(APIView):
    def get(self, request, org_id):
        ...
        token = jwt.encode(
            payload, COOKIE_ENCRYPTION_SECRET, algorithm='HS256')
        response = Response({"message": "Token generated"},
